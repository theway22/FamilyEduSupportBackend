...
public class ShangpinController extends SimpleController {
	...
	public JsonResult list(){
		...
		return JsonResult.success(1,"获取教育服务信息",pageInfo);
	...
	public  JsonResult getTypes(){
		...
		return JsonResult.success(1,"获取 教育服务类别",listLanmu);
	}
...
//		List<ShangpinVo> listRecomment=shangpinSrv.getRecomment(request,temhy);
	...
	public JsonResult addStock() {
		...
		if (spid == null)
			return JsonResult.error(-1,"id不能为空");
		Shangpin shangpin = shangpinSrv.load(new Integer(spid));
		if (shangpin == null)
		...
		if (count != null) {
			shangpin.setKucun(shangpin.getKucun() + Integer.parseInt(count));
